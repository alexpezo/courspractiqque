...
// Version     : pseudo code trouvé sur http://fr.wikipedia.org/wiki/Tri_rapide
//============================================================================
vector<int> QuickSortSeuil(vector<int> donnesFichier, int first, int last)
{
	if(first < last)
	{
		if(first + nbSeuil < last)
		{
			...
			if (first < pivot - 1)
				QuickSortSeuil(donnesFichier, first, pivot-1);
			if (pivot < last)
				QuickSortSeuil(donnesFichier, pivot+1, last);
		}
/*		else
		{
...
int main()
{
	...
	vector<int> result = QuickSortSeuil(donnesFichier, 0, donnesFichier.size()-1);
}
