...
// Version     : pseudo code trouvé sur http://fr.wikipedia.org/wiki/Tri_rapide
//============================================================================
vector<int> QuickSort(vector<int> donnesFichier, int first, int last)
{
	if(first < last)
	{
		int pivot = first;
		...
		if (first < pivot - 1)
			QuickSort(donnesFichier, first, pivot-1);
		if (pivot < last)
			QuickSort(donnesFichier, pivot+1, last);
	}
	return donnesFichier;
}
...
int main()
{
	...
	vector<int> result = QuickSort(donnesFichier, 0, donnesFichier.size()-1);
}
